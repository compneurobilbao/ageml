...
class AgeML:
        ...
        if self.model_type in AgeML.model_dict.keys() and self.hyperparameter_tuning > 0:
            ...
            for hyperparam_name in list(hyperparam_types.keys()):
                ...
                if hyperparam_types[hyperparam_name] == "log":
                    param_grid[f"model__{hyperparam_name}"] = np.logspace(bounds[0], bounds[1], int(self.hyperparameter_tuning))
                elif hyperparam_types[hyperparam_name] == "int":
                    param_grid[f"model__{hyperparam_name}"] = np.rint(
                        np.linspace(bounds[0], bounds[1], int(self.hyperparameter_tuning))
                    ).astype(int)
                elif hyperparam_types[hyperparam_name] == "float":
                    param_grid[f"model__{hyperparam_name}"] = np.linspace(bounds[0], bounds[1], int(self.hyperparameter_tuning))
        else:
    ...
    def set_pipeline(self):
        ...
        if self.feature_extension != 0:
            pipe.append(
                (
                    "feature_extension",
                    preprocessing.PolynomialFeatures(degree=self.feature_extension),
                )
            )
        # Model
    ...
    def fit_age(self, X, y):
        ...
        if self.hyperparameter_grid != {}:
            ...
            opt_pipeline = model_selection.GridSearchCV(
                self.pipeline,
