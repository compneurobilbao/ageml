...
class AgeMLTest(modelling.AgeML):
    def __init__(self, scaler='standard', scaler_params={'with_mean': True},
                 model='linear', model_params={'fit_intercept': True},
                 CV_split=5, seed=42):
        self.scaler = scaler
        ...
        self.seed = seed
        super().__init__(self.scaler, self.scaler_params,
                         self.model, self.model_params,
                         self.CV_split, self.seed)
